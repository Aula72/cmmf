...
<script>
    ...
    const change_grp =(x)=>{
        ...
        let head = ["MID", "Saving", "Social Fund", "Fines","Edn In", "Edn Out", "Subscription","Loan Out", "Loan Payment", "Loan Forms", "Membership", `<div class="dont-print"></div>`]
        var body = []
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(response.reports.length){
                    ...
                    body.push(["Total", nm.format(t_saving), nm.format(t_social), nm.format(t_fine), nm.format(t_edin), nm.format(t_edout), nm.format(t_sub), nm.format(t_lout), nm.format(t_repay), nm.format(t_lform), nm.format(t_mem), `<div class="dont-print"></div>`])
                    // console.log(body)
